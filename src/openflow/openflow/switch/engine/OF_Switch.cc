...
void OF_Switch::initialize(){
  ...
  //  socket.setDataTransferMode(TCP_TRANSFER_OBJECT);
    ...
    for(int i=0; i< interfaceTable->getNumInterfaces() ;i++){
        if(interfaceTable->getInterface(i) != interfaceTable->getInterfaceById(0)){
...
void OF_Switch::processQueuedMsg(cMessage *data_msg){
   ...
   /*     if (dynamic_cast<EthernetMacHeader *>(data_msg) != NULL){ //msg from dataplane
            EthernetMacHeader *frame = (EthernetMacHeader *)data_msg;
            //copy the frame as the original will be deleted
            EthernetMacHeader *copy = frame->dup();
            processFrame(copy);
        }*/
        if (dynamic_cast<Packet *>(data_msg) != NULL){ //msg from dataplane
            Packet *frame = (Packet *)data_msg;
                 //copy the frame as the original will be deleted
                Packet *copy = frame->dup();
                 processFrame(copy);
             }
    } else {
        controlPlanePacket++;
...
//void OF_Switch::processFrame(EthernetMacHeader *frame){
void OF_Switch::processFrame(Packet *packet){
    oxm_basic_match match = oxm_basic_match();
    auto frame = packet->peekAtFront<EthernetMacHeader>();
    //extract match fields
    //match.in_port = frame->getArrivalGate()->getIndex();
    match.in_port = frame->getTag<inet::InterfaceInd>()->getInterfaceId();
    ...
    if(dynamic_cast<inet::IcmpHeader *>(packet->getEncapsulatedPacket()->getEncapsulatedPacket()) != NULL){
        inet::IcmpHeader *icmpMessage = (IcmpHeader *)packet->getEncapsulatedPacket()->getEncapsulatedPacket();
...
/*        PingPayload * pingMsg =  (PingPayload * )icmpMessage->getEncapsulatedPacket();
        ...
        //TODO: add IcmpEchoRequest or reply... ??
    }
   ...
   if (lookup != NULL){
       ...
       ofp_action_output action_output = lookup->getInstructions();
        ...
        if (outport == OFPP_CONTROLLER) {
            ...
            OFP_Packet_In *packetIn = OFMessageFactory::instance()->createPacketIn(OFPR_ACTION_SET, packet);
#endif

            socket.send(packetIn);
        } else {
           //send it out the dataplane on the specific port
           send(packet->dup(), "dataPlaneOut", outport);
       }
   } else {
       // lookup failed
       flowTableMiss++;
       EV << "No Entry Found contacting controller" << '\n';
       handleMissMatchedPacket(packet);
   }
...
void OF_Switch::handleMissMatchedPacket(Packet *packet) {
...
#if OFP_VERSION_IN_USE == OFP_100
        packetIn = OFMessageFactory::instance()->createPacketIn(
                OFPR_NO_MATCH, packet, buffer.storeMessage(packet->dup()), false);
#elif OFP_VERSION_IN_USE == OFP_151
        packetIn = OFMessageFactory::instance()->createPacketIn(
                OFPR_TABLE_MISS, packet, buffer.storeMessage(packet->dup()), false);
#endif
...
void OF_Switch::handlePacketOutMessage(OFP_Message *of_msg){
    ...
    if(bufferId != OFP_NO_BUFFER){
        ...
        frame = dynamic_cast<Packet *>(packet_out_msg->getEncapsulatedPacket());
    }
...
void OF_Switch::executePacketOutAction(ofp_action_output *action_output, Packet *packet, uint32_t inport){
    uint32_t outport = action_output->port;
    take(packet);
    auto frame = packet->peekAtFront<EthernetMacHeader>();
    ...
    if(outport == OFPP_ANY){
        ...
        for (unsigned int i=0; i<n; ++i) {
            if(i != inport && !(portVector[i].state & OFPPS_BLOCKED)){
                send(packet->dup(), "dataPlaneOut", i);
            }
        }
    }else {
        EV << "Send Packet\n" << '\n';
